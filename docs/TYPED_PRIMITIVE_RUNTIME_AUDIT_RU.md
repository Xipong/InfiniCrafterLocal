...
Новый полный panel на прежнем snapshot остановлен после подтверждения дефекта; его partial results не являются acceptance. Свежий clean-head panel после исправления и отдельный C# replay описаны ниже; MP-ограничения остаются открытыми.

## Полный Live20 на `3834acd`: pipeline GREEN, acceptance RED

Полный panel `typed-final-3834acd-flashlite31-all18k-medium-live20` исполнен на clean `3834acd5faff483717568b94a365eb185205f4aa`, модель `gemini-3.1-flash-lite`, все стадии `medium`/18000, parallel crafts=3, без image generation/fallback. Исходный `summary.json` сохранён неизменным (`sha256:13a243ff28e19404dfaa1185bb3337d8faf4dc8de17ea17f0fbf71320a5791eb`).

- 20 уникальных cases и 20 image-boundary contracts; все 20 прошли pipeline, failedCases пуст.
- **Acceptance RED:** `firstAuthorSuccesses=2`, требовалось ≥10; runner завершился exit 1. Эти два случая (`swift_boots`, `astral_minishark`) не требовали Gameplay Repair в итоговой попытке, но оба имели ранее прерванную transport-попытку. Это не 2 полностью безповторных craft.
- 3 case-level transport retries после пауз по 60 s: RemoteDisconnected на Visual `swift_boots`, read timeout на VFX `astral_minishark`, HTTP503 на Author `silt_extractinator`. Скрытых low-level HTTP retries — 0; ledger содержит все 23 craft attempts, 94 logical / 94 HTTP requests. В итоговых 20 результатах 18 Gameplay Repairs; с прерванными попытками всего 19 scoped Repair requests.
- Все реально отправленные стадии подтвердили `reasoning_effort=medium`, `max_tokens=18000`; программного заполнения отсутствующих authored значений не добавлялось.
- На всех 20 итоговых контрактах выполнен отдельный **diagnostic C# FromJson replay: 20 passed, 0 failed**. Использован существующий QA no-image fixture, не image generation. Replay не меняет RED verdict кампании и не доказывает игровой/MP loop. Receipt с hashes: `<campaign>/diagnostic-csharp-replay-receipt.json`; contracts — `diagnostic-replay-contracts.jsonl`.
- GitHub CI `36217123862` успешен; tML build job намеренно пропустил сборку из-за отсутствующих ParticleLibrary/Luminance DLL. Локальная ранее выполненная DLL-сборка остаётся отдельным доказательством, C# runtime этой пачкой не менялся.

Разбор всех итоговых Repair dossiers подтверждён отдельным пересчётом parent: 17 из 18 repaired cases пропускали обязательные params — всего 76 missing-field errors и 37 сопровождающих `shape_one_of` (не 37 отдельных design failures). По полям: `holdoutOffsetX`/`holdoutOffsetY` по 16, `localNpcHitCooldownTicks` 8, `channel` 7, `manaCost`/`craftYield` по 6. В 11 случаях причиной Repair были только пропущенные поля. Дополнительно: 7 semantic diagnostics в 6 случаях и 4 прочие shape diagnostics в 2 случаях. Прерванная первая попытка `astral_minishark` не включена в эти final-case counts.

Все 23 Author requests получили одинаковые compact cards/field guide. Requiredness в guide совпала со schema для 229 параметров 52 capabilities; противоречия требований не обнаружено. В режиме `json_object` полная schema модели не передавалась. Результат показывает недостаточную полноту ответов Author в этой конфигурации, а не основание для host defaults. Минимальная следующая гипотеза — короткая общая проверка полноты выбранных calls непосредственно перед answer shape; эффект требует отдельного matched prompt-only эксперимента, на этом panel не доказан. Успешный Repair не заменяет требование первого Author; подбором удачных повторов или снижением порога этот panel не закрывается.
