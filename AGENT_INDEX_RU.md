# AGENT_INDEX_RU — InfiniCrafterLocal v0.4.239
...
- HTTP entrypoint — `web/server.py`; public diagnostic/tooling API — `web/api.py`.
...
| Runtime engine calls | `core/runtime_authoring/schema.py`, `common.py`, `normalize.py`, `structural.py`, `semantics.py`, `compiler.py`, `reports.py` |
...
| Balance/genome/gameplay | `combine_balance.py`, `combine_genome.py`, `combine_genome_contract.py`, `combine_validation.py`, `combine_gameplay.py` |
| Visual prompts/assets | `visual_prompt_contracts.py`, `visual_asset_plan.py`, `visual_sprite_generation.py`, `visual_asset_manifest.py`, `visual_delivery_gate.py`, `visual_generation_pipeline.py` |
| Sprite postprocess | `sprite_contracts.py`, `sprite_geometry.py`, `sprite_keyer.py`, `sprite_postprocess.py`, `sprite_processing_pipeline.py` |
| Image backend | `image_backend_pipeline.py`, `pipeline_visual_config.py`, `services/sdcpp_backend.py`, `services/sdcpp_service.py` |
| VFX manifest/director | `core/vfx_manifest.py`, `vfx_manifest_config.py`, `vfx_recipe_library.py`, `vfx_director_context.py`, `vfx_director_prompt.py`, `vfx_director_contract.py`, `vfx_composition.py`, `vfx_runtime_slots.py` |
...
| C# runtime execution | `ModSources/InfiniCrafterLocal/Common/Models/GeneratedItemData*.cs`, `Content/Projectiles/GeneratedProjectile*.cs`, `Common/InfiniRuntimeLimits.cs` |
