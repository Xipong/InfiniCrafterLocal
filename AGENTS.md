...
- **Weapon image topology preserves authored structure.** Read `docs/ZIMAGE_WEAPON_TOPOLOGY_RU.md`: code may stop the image backend from inventing unrequested duplicate parts, but it must not impose one grip, one blade, one continuous body, or any other preferred construction. Literal furniture attachments, disassembled parent parts, paired/double-ended forms, and other weird authored structures are all valid. Do not add weapon-name tables, VL judges, or automatic semantic retry loops.
