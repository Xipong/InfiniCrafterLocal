    ...
    *,
    ...
    return {
        ...
        "rules": [
            ...
            "For each repairRequirements row with requiredBindingUpdates, emit every listed existing binding exactly once using one of its complete allowed transactions; mustApplyAll means these updates are one coupled repair, not alternatives. When mustCreateExactlyOne is true, create exactly one allowedBindingTransactions row. When mustChooseExactlyOne is true without mustCreateExactlyOne, emit exactly one complete allowedBindingTransactions choice: update its matching existing binding only when projected by bindingAlternatives, otherwise create it only when repairScope.create.bindings permits; never update every listed lane.",
