...
_ROLE_KIT_PROMPT_FIELDS = {
    ...
    "equip_overlay": "equipOverlayPrompt",
}
...
def visual_asset_runtime_gate(data: dict[str, Any], role: str, authored_mode: str) -> tuple[str, str]:
    ...
    if role == "equip_overlay":
        ...
        if result_kind not in {"armor", "accessory"}:
            return "none", "equip_overlay_requires_equipment_result"
        if mode != "baked_sprite":
            return "none", "equip_overlay_requires_baked_sprite"
        return "baked_sprite", ""
    ...
    if role == "projectile" and mode == "baked_sprite":
        ...
        returning_item_body = runtime_family == "returning" and delivery == "throw"
        if is_item_bodied_projectile_family(runtime_family) and (
            held_item_body or returning_item_body or not distinct_from_item
        ):
            reason = (
                "held_item_bodied_runtime_reuses_item_sprite"
                if held_item_body
                else "returning_item_body_reuses_item_sprite"
                if returning_item_body
                else "item_bodied_runtime_reuses_item_sprite"
            )
    ...
    if role == "child" and mode == "baked_sprite":
        ...
        sentry_shot = runtime_family == "sentry" and bool(str(attack.get("secondaryProjectileShape") or "").strip())
        if int(attack.get("maxChildProjectiles") or 0) <= 0 and not has_child_vfx_slot and not sentry_shot:
...
def apply_visual_asset_runtime_gates(data: dict[str, Any], kit: dict[str, Any]) -> None:
    ...
    for role in ["projectile", "impact", "child", "field", "equip_overlay"]:
...
def build_visual_asset_plan(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if result_kind in {"armor", "accessory"}:
        equip_mode = authored_asset_mode(data, "equip_overlay")
        equip_prompt = _role_asset_prompt(data, "equip_overlay")
        equip_entry = {
            "role": "equip_overlay", "assetId": base + "_equip_overlay", "canvas": 48,
            "prompt": equip_prompt, "required": True, "authoringPolicy": "ai_primary_non_procedural",
            "assetMode": equip_mode or "none", "assetDecisionBy": "model",
        }
        if equip_mode == "baked_sprite":
            plan.append(equip_entry)
        else:
            equip_entry["status"] = "skipped_not_authored_baked"
            equip_entry["skipReason"] = "equipment result requires explicit bakedAssets.equip_overlay.mode=baked_sprite"
            plan.append(equip_entry)
    if isinstance(attack, dict) and attack.get("enabled"):
        ...
        if projectile_mode == "reuse_item_sprite":
            plan.append({"role": "projectile", "assetId": base + "_projectile", "canvas": effective_projectile_canvas(data), "prompt": projectile_prompt, "required": False, "status": "reuses_item_sprite", "skipReason": "runtime asset contract reuses the generated item sprite for this item-bodied executor", "authoringPolicy": "ai_primary_non_procedural", "assetMode": "reuse_item_sprite", "assetDecisionBy": "runtime_asset_contract"})
