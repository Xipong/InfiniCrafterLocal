...
def record_combine_failure(
    ...
    """Persist the latest failed craft without affecting craft semantics."""
    ...
    _LAST_COMBINE_FAILURE.update(failure)
    failure_state.persist_failure(CACHE_DIR, LAST_COMBINE_FAILURE_FILE, failure, log_event)
...
def last_combine_failure_summary() -> dict[str, Any]:
    return failure_state.read_failure_summary(_LAST_COMBINE_FAILURE, LAST_COMBINE_FAILURE_FILE)


def last_combine_failure_payload() -> dict[str, Any]:
    if _LAST_COMBINE_FAILURE:
        return dict(_LAST_COMBINE_FAILURE)
    try:
