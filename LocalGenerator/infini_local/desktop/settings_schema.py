...
FIELD_ORDER = [
    ...
    "INFINI_LLM_API_MODE",
    "INFINI_LLM_POOL_FAILURE_COOLDOWN_SECONDS",
    ...
    "INFINI_LLM_POOL_3_ENABLED",
    "INFINI_LLM_POOL_3_PROVIDER",
    "INFINI_LLM_POOL_3_BASE_URL",
    "INFINI_LLM_POOL_3_API_KEY",
    "INFINI_LLM_POOL_3_MODEL",
    "INFINI_LLM_POOL_3_API_MODE",
    "INFINI_LLM_POOL_4_ENABLED",
    "INFINI_LLM_POOL_4_PROVIDER",
    "INFINI_LLM_POOL_4_BASE_URL",
    "INFINI_LLM_POOL_4_API_KEY",
    "INFINI_LLM_POOL_4_MODEL",
    "INFINI_LLM_POOL_4_API_MODE",
    ...
    "INFINI_CODEX_IMAGE_MODEL",
    "INFINI_CODEX_IMAGE_QUALITY",
    "INFINI_CODEX_IMAGE_SIZE",
    "INFINI_CODEX_IMAGE_TIMEOUT",
    ...
    "INFINI_COMBINE_BUSY_WAIT_SECONDS",
    "INFINI_MULTIDEV_CONCURRENCY",
    "INFINI_DEBUG_ATTACK_CONSUMABLE_MIN_YIELD_ENABLED",
    "INFINI_DEBUG_ATTACK_CONSUMABLE_MIN_YIELD",
...
DEFAULTS = {
    ...
    "INFINI_LLM_API_MODE": "auto",
    "INFINI_LLM_POOL_FAILURE_COOLDOWN_SECONDS": "45",
    "INFINI_LLM_POOL_2_ENABLED": "0",
    "INFINI_LLM_POOL_2_PROVIDER": "openai_compat",
    "INFINI_LLM_POOL_2_BASE_URL": "",
    "INFINI_LLM_POOL_2_API_KEY": "",
    "INFINI_LLM_POOL_2_MODEL": "",
    "INFINI_LLM_POOL_2_API_MODE": "auto",
    "INFINI_LLM_POOL_3_ENABLED": "0",
    "INFINI_LLM_POOL_3_PROVIDER": "openai_compat",
    "INFINI_LLM_POOL_3_BASE_URL": "",
    "INFINI_LLM_POOL_3_API_KEY": "",
    "INFINI_LLM_POOL_3_MODEL": "",
    "INFINI_LLM_POOL_3_API_MODE": "auto",
    "INFINI_LLM_POOL_4_ENABLED": "0",
    "INFINI_LLM_POOL_4_PROVIDER": "openai_compat",
    "INFINI_LLM_POOL_4_BASE_URL": "",
    "INFINI_LLM_POOL_4_API_KEY": "",
    "INFINI_LLM_POOL_4_MODEL": "",
    "INFINI_LLM_POOL_4_API_MODE": "auto",
    "INFINI_LLM_FALLBACK_PROVIDER": "",
    "INFINI_LLM_FALLBACK_MODEL": "",
    "INFINI_LLM_FALLBACK_BASE_URL": "",
    "INFINI_LLM_FALLBACK_API_KEY": "",
    ...
    "INFINI_IMAGE_MAX_CONCURRENCY": "1",
    "INFINI_SDCPP_SERVER_EXE": r"C:\Games\sdcpp-hybrid-gfx1030-134c821\sd-server.exe",
    "INFINI_SDCPP_ROCM_COMPAT_ROOT": r"C:\Games\sdcpp-hybrid-gfx1030-134c821",
    "INFINI_SDCPP_MODEL": r"C:\Games\sdcpp-hybrid-gfx1030-134c821\models\Flux 2 4B\flux-2-klein-4b-Q8_0.gguf",
    "INFINI_SDCPP_VAE": r"C:\Games\sdcpp-hybrid-gfx1030-134c821\models\Flux 2 4B\ae.safetensors",
    "INFINI_SDCPP_LLM": r"C:\Games\sdcpp-hybrid-gfx1030-134c821\models\Flux 2 4B\Qwen 3\Qwen3-4B-UD-Q5_K_XL.gguf",
    "INFINI_SDCPP_LORA_DIR": "",
    "INFINI_SDCPP_LORA_FILE": "",
    "INFINI_SDCPP_LORA_WEIGHT": "0.25",
    "INFINI_SDCPP_LORA_PROMPT_TAGS": "",
    ...
    "INFINI_COMBINE_BUSY_WAIT_SECONDS": "210",
    "INFINI_MULTIDEV_CONCURRENCY": "3",
    "INFINI_DEBUG_ATTACK_CONSUMABLE_MIN_YIELD_ENABLED": "0",
    "INFINI_DEBUG_ATTACK_CONSUMABLE_MIN_YIELD": "10",
    "INFINI_TRACE_PROMPTS": "1",
    "INFINI_TRACE_MAX_PROMPT_CHARS": "18000",
    "INFINI_TRACE_EVENTS_TAIL": "120",
    "INFINI_CONSOLE_EVENT_LEVEL": "warn",
}
...
PRESETS = {
    "OpenAI-compatible + FLUX.2 Klein 4B hybrid": {
        ...
        # Большая strict JSON Schema Gameplay Author не принимается частью
        # OpenAI-compatible провайдеров (в том числе Google Gemini: 400
        # INVALID_ARGUMENT ещё до чтения промпта), а /responses у них может
        # отсутствовать. Форму запроса, которая реально проходит, задаём явно;
        # json_schema остаётся доступным выбором в строке настройки.
        "INFINI_LLM_RESPONSE_FORMAT": "json_object",
        "INFINI_LLM_API_MODE": "chat_completions",
        "INFINI_IMAGE_BACKEND": "sdcpp",
        "INFINI_SDCPP_SERVER_EXE": r"C:\Games\sdcpp-hybrid-gfx1030-134c821\sd-server.exe",
        "INFINI_SDCPP_ROCM_COMPAT_ROOT": r"C:\Games\sdcpp-hybrid-gfx1030-134c821",
        "INFINI_SDCPP_MODEL": r"C:\Games\sdcpp-hybrid-gfx1030-134c821\models\Flux 2 4B\flux-2-klein-4b-Q8_0.gguf",
...
PRESET_HELP = {
    "OpenAI-compatible + FLUX.2 Klein 4B hybrid": "Контракт пишет выбранный OpenAI-compatible API (например Gemini), PNG рисует FLUX.2 Klein 4B: ROCm TE+diffusion, Vulkan VAE, TE weights в CPU RAM; 512x512/4 steps/CFG 1/Euler. prompt_light не отправляет несовместимое API reasoning-поле. Транспорт задан явно: json_object + chat_completions, потому что strict json_schema Gemini отклоняет, а /responses у него нет.",
...
FIELD_HELP = {
    ...
    "INFINI_MULTIDEV_CONCURRENCY": "Сколько админских окон /multidevcraft могут реально работать одновременно. Поддерживается 2 или 3; каждое окно закреплено за LLM с тем же номером без перехода на соседнюю модель.",
    "INFINI_DEBUG_ATTACK_CONSUMABLE_MIN_YIELD_ENABLED": "Debug-only delivery override: повышает количество за один крафт для consumable weapon и ammo. На LLM/prompt и canonical recipe не влияет. Зелья и материалы не меняются.",
    "INFINI_DEBUG_ATTACK_CONSUMABLE_MIN_YIELD": "Минимальное количество атакующего расходника, выдаваемое одним крафтом при включённом debug override. Также поднимает maxStack до этого минимума, иначе Terraria обрежет выдачу.",
    ...
    "INFINI_CONSOLE_EVENT_LEVEL": "Какие события дублировать в окно сервера. cache/events.ndjson всегда пишется полностью; это только эхо в консоль, чтобы упавший крафт был виден сразу, а не только в логе. warn — рекомендуемое, off — как раньше (окно молчит).",
    ...
    "INFINI_LLM_POOL_FAILURE_COOLDOWN_SECONDS": "После provider failure временно исключить profile из выбора новых items; текущий stage сразу пробует следующий profile.",
    ...
    "INFINI_LLM_RESPONSE_FORMAT": "Как просить JSON у модели. json_object — проверенный рабочий выбор для Gemini/OpenRouter. json_schema отклоняется Google Gemini (400 INVALID_ARGUMENT на большой схеме Author) и оставлен опционально; auto для удалённых API тоже разворачивается в json_schema.",
    ...
    "INFINI_IMAGE_BACKEND": "Кто рисует PNG: sd.cpp, Codex OAuth по подписке, внешний Image API, A1111, ComfyUI или выключено.",
    "INFINI_CODEX_IMAGE_MODEL": "Image-модель Codex, не текстовая LLM. По умолчанию gpt-image-2.",
    "INFINI_CODEX_LLM_MODEL": "Текстовая Codex-модель. Обновить список = запросить модели текущего аккаунта; slug можно ввести вручную.",
    "INFINI_CODEX_VISUAL_REASONING": "Reasoning текстового Visual Director, отдельно от image-модели и качества PNG. inherit = наследовать effort основной LLM.",
    "INFINI_CODEX_IMAGE_QUALITY": "Качество PNG по подписке: low, medium, high или auto; влияет на время и квоту.",
    "INFINI_CODEX_IMAGE_SIZE": "Размер исходного PNG. Финальный размер спрайта задаёт Visual Director и общий postprocess.",
    "INFINI_CODEX_IMAGE_TIMEOUT": "Таймаут одного запроса Codex в секундах. Ошибки OAuth/квоты не повторяются автоматически.",
    "INFINI_IMAGE_MAX_CONCURRENCY": "Максимум одновременных запросов ко всем image backend. Для одного общего GPU/sd-server оставь 1; multi-dev LLM lanes при этом могут работать параллельно.",
    ...
    "INFINI_SDCPP_LORA_FILE": "Конкретный файл LoRA (*.safetensors/*.ckpt/*.pt/*.pth). GUI возьмёт родительскую папку для --lora-model-dir и точное имя файла для структурированного lora[] payload.",
    "INFINI_SDCPP_LORA_WEIGHT": "Вес выбранной LoRA в структурированном HTTP payload. Для проверенной pixel-art LoRA безопасная отправная точка 0.20-0.30; 0.45+ может заметно менять форму.",
    "INFINI_SDCPP_LORA_PROMPT_TAGS": "Совместимый ввод <lora:name:weight>. Перед HTTP-вызовом теги удаляются из текста prompt и переводятся в структурированное поле lora[].",
...
OPTION_HELP = {
    ...
    "INFINI_LLM_PROVIDER": {
        "openai_codex": "ChatGPT/Codex OAuth по подписке: текстовый каталог читается из аккаунта. Не API key и не image-модель.",
    ...
    "INFINI_LLM_RESPONSE_FORMAT": {
        "auto": "Дефолт: сервер сам выбирает безопасный формат под провайдера/модель. Для удалённых OpenAI-compatible API это strict json_schema, поэтому на Gemini auto тоже упирается в лимит сложности схемы.",
        "json_schema": "Просить строгую JSON schema. НЕ РАБОТАЕТ на Google Gemini и части OpenAI-compatible шлюзов: большая схема Gameplay Author отклоняется как 400 INVALID_ARGUMENT ещё до чтения промпта. Оставлено опционально для провайдеров, которые её реально держат.",
        "json_object": "Просить JSON object без полной schema. Рабочий выбор для Gemini/OpenRouter: форму ответа задаёт requiredJsonShape в промпте, а авторитетными остаются локальный валидатор и компилятор.",
        "off": "Не отправлять response_format; остаётся только prompt-инструкция вернуть JSON.",
    },
    "INFINI_CONSOLE_EVENT_LEVEL": {
        "warn": "Рекомендуемое: в окно сервера попадают предупреждения и ошибки — отклонённая форма запроса, смена транспорта, упавший крафт. Информационный шум остаётся только в файле.",
        "error": "Только ошибки. Тише, но смена транспорта и отклонённый запрос в консоли не видны.",
        "info": "Плюс информационные события: usage, lease, готовность sd.cpp. Заметно многословнее.",
        "debug": "Всё, включая отладочные события спрайт-пайплайна. Для разбора конкретной проблемы.",
        "off": "Как было раньше: окно показывает только стартовый баннер, все события — только в cache/events.ndjson.",
    },
    "INFINI_LLM_REASONING_MODE": {
        "off": "Не задавать общий effort. Gameplay Author всё равно требует минимум medium (также при none/minimal/low) для сохранения контракта; Visual Director при inherit оставляет effort на усмотрение модели. Это не лимит квоты.",
    ...
    "INFINI_IMAGE_BACKEND": {
        ...
        "openai_codex": "ChatGPT/Codex-подписка через OAuth. Отдельный вход; API key и sd.cpp не используются.",
    ...
    "INFINI_SPRITE_DOWNSCALE_FILTER": {
        "box": "Дефолт: усредняет footprint high-res fake pixel-art в устойчивые кластеры без sharpen-ореолов.",
        "bilinear": "Самый мягкий вариант; может замыливать тонкие предметы.",
