...
def test_balance_mode_has_one_small_exact_owner(monkeypatch) -> None:
...
def test_safety_preserves_authored_damage_and_reports_soft_advice(monkeypatch) -> None:
...
def test_normalize_keeps_legacy_soft_envelope_as_explicit_opt_in(monkeypatch) -> None:
...
def test_report_disables_python_runtime_corridor_but_safety_keeps_it(monkeypatch) -> None:
    raw = {
        "shotCount": 20,
        "pierce": 20,
        "lifetimeTicks": 2000,
...
def test_equipment_budget_can_report_without_mutating() -> None:
...
def test_balance_report_names_active_mode(monkeypatch) -> None:
    monkeypatch.setenv("INFINI_BALANCE_MODE", "safety")
