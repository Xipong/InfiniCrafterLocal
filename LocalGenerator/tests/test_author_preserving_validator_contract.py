...
def _check_authored_damage_is_preserved_inside_broad_safety_envelope() -> None:
    ...
    gp = {"damage": 42}
    assert authored_weapon_damage(gp, fallback=21, max_parent_damage=10, stage=stage, genome=genome, debug=debug) == 42
...
def _check_authored_damage_only_clamps_absurd_outliers() -> None:
    ...
    gp = {"damage": 9999}
    value = authored_weapon_damage(gp, fallback=14, max_parent_damage=10, stage=stage, genome=genome, debug=debug)
    assert value < 9999
...
def _check_non_explosive_magic_aoe_is_not_zeroed_by_family_lock() -> None:
    ...
    g = {
        "delivery": "cast",
        "movement": "straight",
        "movementCode": MOVEMENT_CODE["straight"],
        "effect": "holy",
        "effectCode": EFFECT_CODE["holy"],
        "onHit": "aura_pulse",
        "onHitCode": ONHIT_CODE["aura_pulse"],
        "shotCount": 1,
        "pierce": 0,
        "aoeRadiusTiles": 1.4,
        "lifetimeTicks": 90,
    ...
    out = apply_family_locks_to_genome(g, {"name": "A", "damage": 5}, {"name": "B", "damage": 5}, data, stage)
...
def _check_behavior_digest_stays_raw_flag_based_for_owner_checked_projectiles() -> None:
    digest = projectile_behavior_digest_for_llm({
        "internalName": "Spear",
        "source": "item.shoot",
        "aiStyle": 19,
        "ownerHitCheck": True,
...
def _check_behavior_policy_has_no_item_family_exception_table() -> None:
    ...
    a = behavior_policy_for_prompt(bomb_parent, flower_parent)
    b = behavior_policy_for_prompt(flower_parent, bomb_parent)
...
def _check_parent_projectile_affordance_does_not_rewrite_motion_or_shape() -> None:
    ...
    data = {"debug": {}}
    out = apply_parent_projectile_affordance(genome, parent, {}, set(), data, "ranged")
    assert out["movement"] == "straight"
    assert out["movementCode"] == MOVEMENT_CODE["straight"]
    ...
    out2 = apply_parent_projectile_affordance(opt_in, parent, {}, set(), {}, "ranged")
...
def _contract_check_parent_projectile_reference_is_size_only_not_family_router(monkeypatch):
    ...
    chosen = pa.choose_parent_projectile_size_reference({"id": "small_arrow"}, {"id": "large_held"})
    ...
    data = {"debug": {}}
    out = pa.apply_parent_projectile_affordance(genome, {"id": "small_arrow"}, {"id": "large_held"}, set(), data, "ranged")
    ...
    data2 = {"debug": {}}
    out2 = pa.apply_parent_projectile_affordance(opt_in, {"id": "small_arrow"}, {"id": "large_held"}, set(), data2, "ranged")
