...
@pytest.mark.parametrize(
    ("authored_ammo", "wire_ammo"),
    [("arrows", "arrow"), ("bullets", "bullet")],
)
def test_actual_ammo_aliases_have_explicit_wire_projection(
    authored_ammo: str,
    wire_ammo: str,
) -> None:
    authored = _authored_item(
    ...
    assert report["ok"] is True, report["blockingClaims"]
    assert final_item["gameplay"]["ammoFor"] == wire_ammo
    receipt = _receipt(report, "stats", "ammoFor", "gameplay.ammoFor")
    assert receipt["authoredValue"] == authored_ammo
    assert receipt["compiledValue"] == wire_ammo
    assert receipt["finalActual"] == wire_ammo
